...
These are notes learned from real collaboration sessions. Follow them to avoid repetitive mistakes.

    - **Ignore IME artifacts.** When the user's message contains trailing garbled characters (e.g., repeated Chinese text like "增删改查"), treat them as input noise and ignore them. Do NOT try to interpret them as business intent (e.g., "CRUD").
    - **Confirm the battlefield before acting.** Before editing code, confirm: which branch am I on? Whose fork does it push to? Is this going into a PR or a fresh change? Don't assume.
    - **Don't create unnecessary branches or worktrees.** If review fixes belong to an existing PR, edit on the PR's branch directly — don't stash, switch branches, or create throwaway branches that need cleanup later.
    - **Finish all edits in one go.** When there are multiple review comments to address, read all relevant files, then apply all edits in rapid succession without pausing after each one waiting for approval.
    - **Don't commit unless asked.** Applying code changes is fine; committing, pushing, amending, or force-pushing are version control operations — wait for explicit instruction.
    - **Self-review before reporting done.** After all edits are applied, run `git diff` and verify the changes match the requirements before telling the user it's done.
    - **GitHub PR flow awareness.** Know who owns the PR (user? fork?) and where changes need to land. If a PR is from the user's fork, edits must end up on the fork's branch, not origin.
    - **Worktree isolation is real.** Edits made inside a `git worktree` stay there when you exit — don't use worktrees for quick edits unless the user explicitly requests it.
