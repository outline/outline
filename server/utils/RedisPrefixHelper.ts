...
export class RedisPrefixHelper {
   ...
   * are stored, shared by all entity types. Stored as a sorted set scored by
   * edit sequence, so the highest scoring member is the last to edit.
   *
   * @param id The ID of the entity being edited.
   * @returns the cache key string.
   */
  public static getCollaboratorsKey(id: string) {
    return `collaborators:v2:${id}`;
  }
