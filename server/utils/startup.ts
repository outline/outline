  ...
  try {
    ...
    if (!isEmpty(pending)) {
      if (getArg("no-migrate")) {
        Logger.warn(
          styleText('red', `Database migrations are pending and were not ran because --no-migrate flag was passed.\nRun the migrations with "yarn db:migrate".`)
        );
        process.exit(1);
      } else {
    ...
    if (err.message.includes("ECONNREFUSED")) {
      Logger.warn(
        styleText('red', `Could not connect to the database. Please check your connection settings.`)
      );
    } else {
...
`)
    ...
    Logger.warn(
      `Running Outline in ${styleText('bold',
        "development mode"
      )}. To run Outline in production mode set the ${styleText('bold',
        "NODE_ENV"
