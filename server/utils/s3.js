...
AWS.config.credentials.get(function () {
  ...
  var secretAccessKey = AWS.config.credentials.secretAccessKey || AWS_SECRET_ACCESS_KEY;
});
...
export const makeCredential = () => {
  const credential =
    AWS.config.credentials.accessKeyId + '/' + format(new Date(), 'YYYYMMDD') + '/' + AWS_REGION + '/s3/aws4_request';
  return credential;
};

export const makePolicy = (credential: string, longDate: string, acl: string) => {
...
export const getSignature = (policy: any) => {
  const kDate = hmac('AWS4' + AWS_SECRET_ACCESS_KEY, format(new Date(), 'YYYYMMDD'));
...
export const uploadToS3FromBuffer = async (buffer: Buffer, contentType: string, key: string, acl: string) => {
  await s3
    .putObject({
      ACL: acl,
...
export const uploadToS3FromUrl = async (url: string, key: string, acl: string) => {
  try {
