...
describe("anchorResolver", () => {
  describe("resolveAnchorToProseMirror", () => {
    ...
    it("should resolve valid anchor positions", () => {
      ...
      // Encode to base64
      const fromB64 = Buffer.from(Y.encodeRelativePosition(fromRelPos)).toString("base64");
      const toB64 = Buffer.from(Y.encodeRelativePosition(toRelPos)).toString("base64");
