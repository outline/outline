...
describe("DocumentMovedProcessor", () => {
  ...
  it("should remove the creator's root membership when a private document is nested under another", async () => {
    const team = await buildTeam();
    ...
    const childMembership = await UserMembership.create({
      userId: user.id,
      documentId: child.id,
      permission: DocumentPermission.Admin,
      index: "b",
    ...
    expect(memberships.length).toBe(1);
    expect(memberships[0].sourceId).toBe(parentMembership.id);
    expect(memberships[0].id).not.toBe(childMembership.id);

    // the root membership on the parent is untouched
    const parentMemberships = await UserMembership.findAll({
      where: { documentId: parent.id, userId: user.id },
    });
