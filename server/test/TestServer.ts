...
class TestServer {
  ...
  post(path: string, context?: User | Team | any, options: any = {}) {
    if (context instanceof User) {
      options.headers = {
        ...options.headers,
        host: env.isCloudHosted
          ? `${context.team.subdomain}.outline.dev`
          : this.address,
      };
      options.body = {
        ...options.body,
        token: context.getJwtToken(),
      };
    } else if (context instanceof Team) {
      options.headers = {
        ...options.headers,
        host: env.isCloudHosted
          ? `${context.subdomain}.outline.dev`
          : this.address,
      };
    } else {
      options = context;
