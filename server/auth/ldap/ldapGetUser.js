...
function getUser(identifier, password){
    return new Promise((resolve, reject)=> {
        ...
        var opts = {
            filter: (process.env.LDAP_USERS_FILTER || "uid={0}").replace("{0}", identifier),
            scope: 'sub',
        ...
        var authed = false;
        client.bind(process.env.LDAP_ADMIN_USER, process.env.LDAP_ADMIN_PASSWORD).then(function(){ //Login to LDAP
            authed = true;
        ...
        }).then(function (result) { //Find user
            var results = [];
            ...
            return results[0];
        }).then(function(user){ //find groups of user
            return getGroups(user.dn, user);
        }).then(function(user){ //Try login as user
            result = user;
            if(shouldTry) {
...
function getGroups(userDN, user){
    return new Promise((resolve, reject)=> {
        var client = new LdapClient({url: process.env.LDAP_URL});
        var opts = {
            filter: (process.env.LDAP_GROUPS_FILTER || "(&(member={0})(objectclass=groupOfNames))").replace("{0}", userDN),
            scope: 'sub',
        ...
        client.bind(process.env.LDAP_ADMIN_USER, process.env.LDAP_ADMIN_PASSWORD).then(function(){ //Login to LDAP
            ...
            return client.search(scope, opts);
        }).then(function (result) { //Find group
            var results = [];
            result.entries.forEach(entry => {
