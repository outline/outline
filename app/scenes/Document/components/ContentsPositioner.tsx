...
const ContentsPositioner = ({
  ...
  React.useLayoutEffect(() => {
    fullWidthElementsRef.current = fullWidthElems;
    handlePositioning();
  }, [fullWidthElems, handlePositioning]);
  ...
  React.useEffect(() => {
    ...
    if (!fullWidthElems.length) {
      positionerRef.current.style.marginTop = InitialMarginTop;
    } else {
      fullWidthElems.forEach((elem) => fullWidthElemsObserver.observe(elem));
    }

    return () => fullWidthElemsObserver.disconnect();
  }, [headings, fullWidthElems, handlePositioning]); // when headings change, contents box size changes.
