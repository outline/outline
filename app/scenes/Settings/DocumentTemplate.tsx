...
/**
 * Editing one of the printable documents.
 *
 ...
 */
export function DocumentTemplateSettings({ type, title, description }: Props) {
  const { t } = useTranslation();
  const fetchAll = useShop((state) => state.fetchAll);
  const templates = useShop((state) => state.documentTemplates);
  const saveDocumentTemplate = useShop((state) => state.saveDocumentTemplate);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const result = await saveDocumentTemplate(draft);
      setNotice(
        result?.saved
          ? t("Saved.")
