import { useEffect } from "react";
import type { StaticContext } from "react-router";
import { useHistory } from "react-router";
import type { RouteComponentProps } from "react-router-dom";
import type { SidebarContextType } from "~/components/Sidebar/components/SidebarContext";
import { useSplitView } from "~/components/SplitView/context";
import { useTrackLastVisitedPath } from "~/hooks/useLastVisitedPath";
import useStores from "~/hooks/useStores";
import { patchLocation } from "~/utils/history";
import { getFocusedSplitPane } from "~/utils/splitView";
import DataLoader from "./components/DataLoader";
import Note from "./components/Note";
import { Footer } from "./components/Footer";
type Params = {
  noteSlug: string;
  revisionId?: string;
};
type LocationState = {
  title?: string;
  restore?: boolean;
  revisionId?: string;
  sidebarContext?: SidebarContextType;
};
type Props = RouteComponentProps<Params, StaticContext, LocationState>;
export default function NoteScene(props: Props) {
  const { ui } = useStores();
  const history = useHistory();
  const { pane, isSplitView } = useSplitView();
  const { noteSlug, revisionId } = props.match.params;
  const currentPath = props.location.pathname;
  useTrackLastVisitedPath(currentPath);
  useEffect(
    () => () => {
      // In a split view only the focused pane owns the active note, so an
      // unfocused pane unmounting must not clear the focused pane's state.
      if (!isSplitView || pane === getFocusedSplitPane()) {
        ui.clearActiveNote();
      }
    },
    [ui, isSplitView, pane]
  );
  useEffect(() => {
    // When opening a note directly on app load, sidebarContext will not be set.
    if (!props.location.state?.sidebarContext) {
      history.replace(
        patchLocation(props.location, {
          state: { ...props.location.state, sidebarContext: "notebooks" }, // optimistic preference of "notebooks"
        })
      );
    }
  }, [props.location, history]);
  // the urlId portion of the url does not include the slugified title
  // we only want to force a re-mount of the note component when the
  // note changes, not when the title does so only this portion is used
  // for the key.
  const urlParts = noteSlug ? noteSlug.split("-") : [];
  const urlId = urlParts.length ? urlParts[urlParts.length - 1] : undefined;
  // Normalize the key so that it is *stable* between renders.
  // Without this, the initial value can be "<urlId>/undefined" and then flip to
  // "<urlId>/" when React stringifies `undefined` on the next render, causing a
  // full unmount/mount cycle of the note subtree. Keeping the key constant
  // prevents extra network requests and preserves editor state on resize.
  const key = revisionId ? `${urlId}/${revisionId}` : urlId;
  return (
    <DataLoader
      key={key}
      match={props.match}
      history={props.history}
      location={props.location}
    >
      {(rest) => (
        <Note {...rest}>
          <Footer note={rest.note} />
        </Note>
      )}
    </DataLoader>
  );
}
