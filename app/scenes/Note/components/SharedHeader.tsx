...
function SharedNoteHeader({ note }: Props) {
  ...
  const { hasHeadings } = useNoteContext();
  ...
  const editAction = can.update ? (
    <Action>
      <Tooltip
        content={t("Edit {{noun}}", { noun: note.noun })}
        shortcut="e"
        placement="bottom"
      >
        <Button
      ...
      actions={
        <>
          ...
          {allowSubscriptions !== false && !user && env.EMAIL_ENABLED && (
            <SubscribeAction shareId={shareId} noteId={note.id} />
          )}
          <AppearanceAction />
          {can.update && share && (
            <Action>
...
const TocButton = styled(Button)<{
  ...
  `}
`;
export default observer(SharedNoteHeader);
