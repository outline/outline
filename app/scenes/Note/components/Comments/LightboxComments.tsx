...
type Props = {
  ...
  note: Note;
  /** The position of the image node in the note. */
  pos: number;
};
function LightboxComments({ note, pos }: Props) {
  const { comments } = useStores();
  const { editor, focusedCommentId, setFocusedCommentId } = useNoteContext();
  const user = useCurrentUser();
  const { t } = useTranslation();
  const can = usePolicy(note);
  const [draft, onSaveDraft] = usePersistedState<ProsemirrorData | undefined>(
    `draft-${note.id}-image-${pos}`,
    undefined
  );
  ...
  // into the note, so instead a pending anchor is set and the server
