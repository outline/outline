...
function Comments() {
  const { ui, comments, notes } = useStores();
  const { pane } = useSplitView();
  const user = useCurrentUser();
  const { editor, isEditorInitialized, setFocusedCommentId } = useNoteContext();
  ...
  useKeyDown("Escape", () => note && ui.setRightSidebar(null, pane));
  ...
  const sortOption: CommentSortOption = user.getPreference(
    UserPreference.SortCommentsByOrderInNote
  )
    ? {
  ...
  const threads = !note
    ? []
    : viewingResolved
      ? comments.resolvedThreadsInNote(note.id, sortOption)
      : comments.unresolvedThreadsInNote(note.id, sortOption);
  ...
  useEffect(() => {
    ...
    const readyToDisplay = Boolean(note && isEditorInitialized);
    if (
      readyToDisplay &&
  ...
  }, [sortOption.type, note, isEditorInitialized, viewingResolved]);
  useEffect(() => {
    setShowJumpToRecentBtn(false);
