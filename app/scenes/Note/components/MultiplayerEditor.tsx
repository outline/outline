...
function MultiplayerEditor(
  ...
  useLayoutEffect(() => {
    ...
    provider.on("synced", () => {
      presence.touch(noteId, currentUser.id, false);
  ...
  }, [history, t, noteId, ui, presence, ydoc, currentUser.id, isMounted, auth]);
  const user = useMemo(
    () => ({
      id: currentUser.id,
  ...
  // while the collaborative note is loading, we render a version of the
  // note from the last text cache in read-only mode if we have it.
...
/**
 * Whether the last known content of the note is worth displaying while the
 * collaborative note loads. An empty note, such as one that was just
