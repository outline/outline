...
export default class CollectionsStore extends BaseStore<Collection> {
  ...
  @computed
  get active(): Collection | undefined | null {
  ...
  async update(params: any): Promise<Collection> {
  ...
  getPathForDocument(documentId: string): DocumentPath | undefined | null {
    return this.pathsToDocuments.find((path) => path.id === documentId);
  }

  titleForDocument(documentUrl: string): string | undefined | null {
    const path = this.pathsToDocuments.find((path) => path.url === documentUrl);
    if (path) return path.title;
  }

  getByUrl(url: string): Collection | undefined | null {
