...
export default class BaseStore<T extends BaseModel> {
  ...
  add = (item: any): T => {
    ...
    if (!(item instanceof Model)) {
      const existing: T | undefined | null = this.data.get(item.id);
      if (existing) {
        set(existing, item);
        return existing;
      } else {
  ...
  @action
  async delete(item: T, options: any = {}) {
  ...
  @action
  fetchPage = async (params?: PaginationParams | null): Promise<any> => {
