...
import { isNoteUrl, isInternalUrl } from "@shared/utils/urls";
  ...
  const handleClickLink = useCallback(
    (href: string, event?: MouseEvent) => {
      ...
      if (isHash(href)) {
        ...
        // when scrolling to an anchor within the same note. The hash (and
      ...
      if (isInternalUrl(href)) {
        ...
        if (
          ...
          (navigateTo.includes("/doc/") || navigateTo.includes("/notebook/")) &&
          !navigateTo.includes(shareId)
        ) {
          navigateTo = sharedModelPath(shareId, navigateTo);
        }
        if (isNoteUrl(navigateTo)) {
          const note = notes.get(navigateTo);
          if (note) {
        ...
        if (
          ...
          history.push(navigateTo, { sidebarContext: "notebooks" }); // optimistic preference of "notebooks"
        } else if (
          Desktop.isElectron() &&
          isModKey(event) &&
          !event.shiftKey &&
