...
function registerConnectionHandlers(
  socket: SocketWithAuthentication,
  { auth }: Pick<RootStore, "auth">
) {
...
function registerEntityHandlers(
  socket: SocketWithAuthentication,
  {
    ...
    policies,
    memberships,
  }: Pick<RootStore, "documents" | "collections" | "policies" | "memberships">
) {
  socket.on(
    "entities",
  ...
  socket.on(
    ...
    async (event: PartialExcept<UserMembership, "id">) => {
      ...
      if (event.userId === currentUserId) {
        invalidateChildPolicies(event.documentId!, stores);
      }
...
function registerCollectionHandlers(
  socket: SocketWithAuthentication,
  stores: Pick<
    RootStore,
    | "collections"
  ...
  const { collections, documents, policies, memberships, groupMemberships } =
    stores;

  socket.on("collections.create", (event: PartialExcept<Collection, "id">) => {
    collections.add(event);
  });

  socket.on("collections.update", (event: PartialExcept<Collection, "id">) => {
    collections.add(event);
  });
...
function registerCommentHandlers(
  socket: SocketWithAuthentication,
  { comments, policies }: Pick<RootStore, "comments" | "policies">
) {
  ...
  socket.on("comments.add_reaction", (event: WebsocketCommentReactionEvent) => {
...
function registerGroupHandlers(
  socket: SocketWithAuthentication,
  { groups, groupUsers }: Pick<RootStore, "groups" | "groupUsers">
) {
...
function WebsocketProvider({ children }: React.PropsWithChildren<object>) {
  ...
  useEffect(() => {
    ...
    function createConnection() {
      ...
      registerConnectionHandlers(currentSocket, stores);
      registerEntityHandlers(currentSocket, stores);
      registerDocumentHandlers(currentSocket, stores, currentUserId);
      registerCollectionHandlers(currentSocket, stores, currentUserId);
      registerCommentHandlers(currentSocket, stores);
      registerGroupHandlers(currentSocket, stores);

      currentSocket.on("teams.update", (event: PartialExcept<Team, "id">) => {
      ...
      currentSocket.on("pins.create", (event: PartialExcept<Pin, "id">) => {
        pins.add(event);
      });

      currentSocket.on("pins.update", (event: PartialExcept<Pin, "id">) => {
        pins.add(event);
      });

      currentSocket.on("pins.delete", (event: WebsocketEntityDeletedEvent) => {
        pins.remove(event.modelId);
      });

      currentSocket.on("stars.create", (event: PartialExcept<Star, "id">) => {
        stars.add(event);
      });

      currentSocket.on("stars.update", (event: PartialExcept<Star, "id">) => {
        stars.add(event);
      });

      currentSocket.on("stars.delete", (event: WebsocketEntityDeletedEvent) => {
      ...
      currentSocket.on("users.update", (event: PartialExcept<User, "id">) => {
        users.add(event);
      });

      currentSocket.on(
        "users.demote",
        async (event: PartialExcept<User, "id">) => {
      ...
      currentSocket.on("users.delete", (event: WebsocketEntityDeletedEvent) => {
      ...
      currentSocket.on("join", (event) => {
        currentSocket?.emit("join", event);
      });

      // received a message from the API server that we should request
      // to leave a specific room. Forward that to the ws server.
      currentSocket.on("leave", (event) => {
        currentSocket?.emit("leave", event);
      });
