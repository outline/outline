...
/** A note as represented within the search index. */
export interface SearchIndexNote {
  id: string;
  /** Display title, already defaulted for notes without one. */
  title: string;
  /** Plain text content of the note, may be absent until it has loaded. */
  text?: string;
  url: string;
  icon?: string | null;
  color?: string | null;
}
/** A single note matched by a search of the index. */
...
const options: IFuseOptions<SearchIndexNote> = {
  keys: [
    { name: "title", weight: 2 },
    { name: "text", weight: 1 },
  ],
 ...
 * decides *which* notes rank, but only literal matches are worth
 * highlighting, as the underlying fuzzy indices span scattered characters and
 * read as noise.
 *
 * @param text the text to search within.
 * @param query the search query.
 * @returns the matched ranges as start/end offsets, in note order.
 */
...
/**
 * Builds a preview snippet of the note content, trimmed to a short window.
...
function buildContext(
  ...
  // the note still makes a useful preview.
  if (!ranges.length) {
...
/**
 * An in-memory fuzzy search index over a set of notes, backed by Fuse.js.
...
export class SearchIndex {
  private records = new Map<string, SearchIndexNote>();
  private fuse = new Fuse<SearchIndexNote>([], options);
  /**
   * Merges notes into the index, rebuilding the collection only when
   ...
   */
  public update(notes: SearchIndexNote[]): boolean {
    let changed = false;
    for (const incoming of notes) {
      const existing = this.records.get(incoming.id);
      const merged: SearchIndexNote = existing
        ? { ...existing, ...incoming }
        : incoming;
  ...
  /**
   * Removes all notes from the index.
   */
  public clear(): void {
    this.records.clear();
   ...
   *
   * @param query the search query.
   * @returns the matching notes ordered by relevance.
   */
  public search(query: string): SearchIndexResult[] {
    ...
    // Building context scans the full text of a note, so it is deferred
    ...
    return this.fuse
      ...
      .map(({ note, score }) => ({
        note,
        score,
        context: buildContext(note.text, trimmed),
      }));
  }
}
