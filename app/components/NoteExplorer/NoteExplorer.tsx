...
import NoteExplorerNode from "./NoteExplorerNode";
import NoteExplorerSearchResult from "./NoteExplorerSearchResult";
...
function NoteExplorer({
  ...
  const normalizedBaseDepth =
    (baseDepth === Infinity ? 0 : baseDepth) + (showNotes ? 0 : 1);
  ...
  React.useEffect(() => {
    notebooks.orderedData
      .filter((notebook) => expandedNodes.includes(notebook.id) || searchTerm)
      .forEach((notebook) => {
        void notebook.fetchNotes();
      });
  }, [notebooks, expandedNodes, searchTerm]);
  ...
  const hasChildren = (node: number) =>
    ...
    (showNotes !== false && nodes[node].type === "collection");
  ...
  const ListItem = observer(
    ({
      ...
      const isNotebook = node.type === "collection";
      let renderedIcon,
        title: string,
        ...
        path;
      if (isNotebook) {
        const col = notebooks.get(node.notebookId as string);
        renderedIcon = col && (
          <CollectionIcon notebook={col} expanded={isExpanded(index)} />
        );
        title = node.title;
      } else {
...
const ListContainer = styled.div`
  ...
  `}
`;
export default observer(NoteExplorer);
