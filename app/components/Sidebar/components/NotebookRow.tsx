...
import { NotebookValidation, NoteValidation } from "@shared/validations";
...
export type NotebookRowProps = {
  ...
  /** Content rendered after the row (e.g. NotebookLinkChildren). */
...
function NotebookRow({
  ...
  const iconElement = icon ?? (
    <CollectionIcon notebook={notebook} expanded={expanded} />
  );
  ...
  return (
    <ActionContextProvider value={{ activeModels: [notebook] }}>
      <Relative ref={mergedRef}>
        <DropToImport notebookId={notebook.id}>
          {sidebarLinkElement}
        </DropToImport>
      </Relative>
        ...
        <SidebarLink
          ...
          label={
            <EditableTitle
              ...
              maxLength={NoteValidation.maxTitleLength}
...
export default observer(NotebookRow);
