...
export type NoteRowProps = {
  /** Note identifier for policy, prefetch and import. */
  noteId: string;
  /** Loaded note; used for editing title and active matching. */
  note?: Note;
  ...
  isDragging?: boolean;
  /** Whether the row's note is being moved. */
  isMoving?: boolean;
...
function NoteRow({
  ...
  const defaultIsActive = React.useCallback(
    (
      ...
      return (note && location.pathname.endsWith(note.urlId)) || !!m;
    },
    [sidebarContext, note]
  );
  ...
  const withImport = noteId ? (
    <DropToImport noteId={noteId}>{sidebarLinkElement}</DropToImport>
  ) : (
    sidebarLinkElement
  );
  return (
