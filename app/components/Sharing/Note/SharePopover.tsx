...
import { notePath, urlify } from "~/utils/routeHelpers";
...
function SharePopover({
  ...
  const [permission, setPermission] = React.useState<NotePermission>(
    NotePermission.Read
  );
  ...
  const rightButton = picker ? (
    pendingIds.length ? (
      <PermissionAction
        ...
        action={inviteAction}
        onChange={(value: NotePermission) => setPermission(value)}
      />
    ) : null
  ) : (
    <CopyLinkButton
      key="copy-link"
      url={urlify(notePath(note))}
