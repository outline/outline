...
function PublicAccess(
  ...
  const noteAbilities = usePolicy(note);
  const canPublish = share ? can.update : noteAbilities.share;
  ...
  const shareUrl =
    sharedParent?.url && !note.isDraft
      ? `${sharedParent.url}${note.url}`
      : (share?.url ?? "");
  const copyButton = (
  ...
  return (
    <div ref={ref}>
      <ListItem
        ...
        subtitle={
          <>
            {sharedParent && !note.isDraft ? (
              sharedParent.notebookId ? (
                <Trans>
                  Anyone with the link can access because the containing
                  notebook,{" "}
                  <UnderlinedLink to={`/notebook/${sharedParent.notebookId}`}>
                ...
                <Trans>
                  ...
                  <UnderlinedLink to={`/doc/${sharedParent.noteId}`}>
                    {sharedParent.sourceTitle}
                  </UnderlinedLink>
                  , is shared
                </Trans>
        ...
        actions={
          sharedParent && !note.isDraft ? null : (
            <Switch
      ...
      <ResizingHeightContainer>
        {sharedParent?.published && !note.isDraft ? (
        ...
        {share?.published && !share.includeChildNotes ? (
