...
function PinnedNotes({
  ...
  const [items, setItems] = useState(pins.map((pin) => pin.noteId));
  ...
  useEffect(() => {
    setItems(pins.map((pin) => pin.noteId));
  }, [pins]);
        ...
        {isCollapsed && (
          <Chips>
            {items.map((noteId) => {
              ...
              return note ? <NoteChip key={noteId} note={note} /> : null;
            })}
          </Chips>
        )}
                ...
                {showPlaceholder ? (
                  ...
                  <AnimatePresence initial={false}>
                    {items.map((noteId) => {
                      ...
                      // Once any note is loaded, never render the placeholder again
...
`;
const ToggleButton = styled(NudeButton)<{
  $visible: boolean;
}>`
  position: absolute;
  top: 0;
...
const List = styled.div<{
  ...
  `};
`;
export default observer(PinnedNotes);
