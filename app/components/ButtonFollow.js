...
class ButtonFollow extends React.Component<Props> {
  ...
  componentDidMount() {
    this.props.follows.fetchPage({
      requestedDocId: this.props.requestedDoc.id,
    });
  }
  ...
  renderFollows() {
    ...
    if (requesteDocFollows.length === 0) {
      ...
      const userFollow = requesteDocFollows.find(
        (follow) => follow.userId === userCurrent
      );
      if (userFollow) {
        return (
          <div>
  ...
  render() {
    return <Actions>{this.renderFollows()}</Actions>;
  }
}
