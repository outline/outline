...
type Props = {
  /** The notebook to import the notes into. */
  notebookId?: string | null;
  /** The note to import the notes as children of. */
  noteId?: string;
  /** Called once files have been chosen and the import has started. */
  onSubmit: () => void;
};
/**
 * A dialog that accepts notes to import, either dropped onto it or chosen
 * with the system file picker, and lists the file formats that are supported.
 */
export const ImportNoteDialog = observer(function ImportNoteDialog({
  notebookId,
  noteId,
  onSubmit,
}: Props) {
  ...
  const { handleFiles } = useImportNote(notebookId, noteId);
  ...
  const handleDropAccepted = React.useCallback(
    (files: File[]) => {
      ...
      // and a successful import navigates to the new note.
      onSubmit();
